...
class User extends SimbioModel {
  ...
  public function add(&$simbio, $str_args) {
    ...
    if (!$str_args) {
      ...
      $simbio->addInfo('USER_ADD_RECORD_INFO', __('You are going to add user'));
      ...
      $_form->submitName = 'add';
      $_form->submitAjax = true;
      $_form->submitValue = __('Add Role');

      // define roles fields
      $_fields['role_name'] = array('id' => 'role_name', 'label' => __('Role Name'), 'type' => 'text', 'required' => true);
      $_fields['privileges'] = array('id' => 'privileges', 'label' => __('Privileges'), 'type' => 'content', 'content' => $this->privilegesList($simbio));
      // add form and set form field value
      foreach ($_fields as $_elm) {
        $_form->add($_elm);
      }
      $simbio->addInfo('ROLE_ADD_RECORD_INFO', __('You are going to add role'));
  ...
  public function index(&$simbio, $str_args) {
    ...
    $simbio->headerBlockMenu = array(
        array('class' => 'add', 'link' => 'user/add', 'title' => __('Add New User'), 'desc' => __('Add new User')),
        array('class' => 'list', 'link' => 'user/manage', 'title' => __('User List'), 'desc' => __('View list of existing User'))
      );
    // build search form
  ...
  public static function login(&$simbio, $str_args) {
    ...
    if (isset($_POST['username']) && isset($_POST['password']) && !User::isUserLogin()) {
      ...
      if ($_user_check->num_rows) {
        ...
        $simbio->addInfo('USER_LOGIN_SUCCESS', sprintf(__('Welcome to %s'), $_global['app_title']));
        ...
        $simbio->addError('USER_LOGIN_FAILED', 'Incorrect username or password. Please supply correct username and password!');
