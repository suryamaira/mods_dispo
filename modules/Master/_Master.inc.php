...
class Master extends SimbioModel {
    ...
    public function index(&$simbio, $str_args) {
        ...
        $simbio->headerBlockMenu = array(
                array('class' => 'add', 'link' => 'master/add/'.$this->dbTable, 'title' => __('Add Master data'), 'desc' => __('Add new Master data')),
                array('class' => 'list', 'link' => 'master/index/'.$this->dbTable, 'title' => __('Data List'), 'desc' => __('View list of existing Master data'))
            );
        // build search form
    ...
    public function menu(&$simbio, $str_menu_type = 'navigation', $str_current_module = '', $str_current_method = '') {
        ...
        if ($str_menu_type == 'main') {
            $_menu[] = array('link' => 'admin/master', 'name' => __('Master File'), 'description' => __('Manages application master file tables.'));
        } else {
    ...
    public function update(&$simbio, $str_args) {
        ...
        $simbio->addInfo('UPDATE_RECORD_INFO', __('You are going to update Master file data'));
        $simbio->loadView($_form, 'MASTER_FILE_FORM');
    }
